...
// type CellSize = (f32, f32);
// const CELL_SIZE: CellSize = (15.0, 15.0);
...
use structs::pixel::Pixel;

fn display_fps(canvas: &mut Canvas, ctx: &mut Context) {
...
struct MainState {
    pub blocks: HashMap<GridPosition, Box<dyn Block>>,
...
impl MainState {
    pub fn new(_ctx: &mut Context) -> MainState {
        let mut blocks: HashMap<GridPosition, Box<dyn Block>> =
            HashMap::new();
        let cell_size = CellSize::new(15.0, 15.0);

        let sand = Sand::new(GridPosition::new(1, 1, cell_size));
        blocks.insert(sand.get_position(), Box::new(sand));
...
impl EventHandler for MainState {
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        // Calculate new positions without updating blocks
        let new_positions: HashMap<GridPosition, GridPosition> = self
            .blocks
            .iter()
            .map(|(position, _block)| {
                let new_position =
                    *position + GridPosition::new(0, 1, self.cell_size);
                (position.clone(), new_position)
            })
            .collect();

        // Check if new positions are valid and update blocks
        for (old_position, new_position) in new_positions.iter() {
            
            if new_position.is_offscreen(ctx)
                || self.position_occupied(*new_position)
            {
                continue;
            }

            if let Some(block) = self.blocks.get_mut(old_position) {
...
fn main() {
    let (mut ctx, event_loop) =
        ContextBuilder::new("Sand Simulation v3", "")
            .window_setup(
                ggez::conf::WindowSetup::default()
                    .title("Sand Simulation v3"),
            )
            .build()
